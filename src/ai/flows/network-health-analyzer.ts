...
const NetworkHealthOutputSchema = z.object({
  overallHealth: z
    .string()
    .describe(
      'An overall assessment of the network health (e.g., "Healthy", "Degraded", "Critical").'
    ),
  potentialIssues: z
    .array(z.string())
...
Provide the output in JSON format. The overallHealth should be one of "Healthy", "Degraded", or "Critical".
`,
});
